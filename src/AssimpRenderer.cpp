...
AssimpRenderer::AssimpRenderer(const std::string file) {
	// maybe test if file exists?
	ifstream is(file);
	if (!is.is_open()) {
		throw new FileNotFound("Specified file not found");
	}
	
	auto scene = m_importer.ReadFile(file, aiProcessPreset_TargetRealtime_Quality);
	if (!scene) {
		throw new LoadFileException("Assimp couldn't load file");
	}
...
void AssimpRenderer::genVAOsAndUniformBuffer(const aiScene *scene) {
	GLuint buffer;
	AssimpMesh mesh;
...
void AssimpRenderer::recursiveRender(RenderProperties &props, const aiScene *sc, const aiNode *node) {
...
void AssimpRenderer::render(RenderProperties &props) {
